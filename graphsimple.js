...
export class graph {
    ...
    constructor(idsvg, conf=null){
        conf = {...defaults, ...conf};

        for (var index in conf) this[index] = conf[index];

        if (idsvg == null) this.idsvg = "#" + newSvg();
        else this.idsvg = idsvg;
        
        this.svg = d3.select(this.idsvg)
            .classed("gs", true);

        if ('class' in this) this.svg.classed(this.class, true);
        if ('viewBox' in this) this.svg.attr('viewBox', this.viewBox);
    ...
    tracer (donnees, fonctionx, fonctiony){
        ...
        if (!this.autoScale) this.Tracer(donnees, fonctionx, fonctiony);
    ...
    vectDraw(vect){
        ...
        vect.ligne = this.annotationsGroup.append("line")
            .attr("x1", this.echellex(vect.x1))
            .attr("x2", this.echellex(vect.x2))
